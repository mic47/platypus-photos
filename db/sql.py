...
from db.types import FeaturePayload, Image, ImageAggregation
...
CREATE INDEX IF NOT EXISTS features_idx_last_update ON features (last_update);
    ...
    def undirty(self, path: str, types: t.List[str], max_last_update: float) -> None:
        q = ", ".join(f"'{qt}'" for qt in types)
        query, param = (
            f"UPDATE features SET dirty = 0 WHERE dirty > 0 AND file = ? AND type in ({q}) AND last_update <= ?",
            (path, max_last_update),
        )
        self._con.execute(query, param)
        self._con.commit()

    def dirty_files(self, types: t.List[str]) -> t.Iterable[t.Tuple[str, int]]:
        if types:
            q = ", ".join(f"'{qt}'" for qt in types)
...
WHERE excluded.feature_last_update > gallery_index.feature_last_update""",
    ...
    def _matching_query(
        self, select: str, url: UrlParameters
    ...
    def get_aggregate_stats(self, url: UrlParameters) -> ImageAggregation:
        ...
        (select, variables) = self._matching_query(
            "tags, classifications, address_name, address_country", url
        )
        query = f"""
...
SELECT "addrc", address_country, COUNT(1) FROM matched_images WHERE address_country IS NOT NULL GROUP BY address_country
        ...
        while True:
            ...
            if not items:
                return ImageAggregation(total, address_cnt, tag_cnt, classifications_cnt)
            for type_, value, count in items:
                if type_ == "total":
                    total = count
                elif type_ == "cls":
                    if not value:
        ...
        while True:
            ...
            for (
                ...
                yield Image(
                    ...
                    None if not tags else dict(zip(tags.split(":"), map(float, tags_probs.split(":")))),
...
class Table:
    def __init__(self, path: t.Union[str, sqlite3.Connection]) -> None:
        ...
        self.gallery_index = GalleryIndexTable(self._con)
        self._init_db()
