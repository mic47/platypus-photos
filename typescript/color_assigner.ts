export class ColorAssigner {
    ...
    private add(label: string): number {
        ...
        const initial_width = 360 - this.sortedColors[this.sortedColors.length - 1].hue + this.sortedColors[0].hue
        ...
        for (let i = 0; i < this.sortedColors.length - 1; i++) {
            const width = this.sortedColors[i+1].hue - this.sortedColors[i].hue
