...
async def _image_watcher_server_loop(reader: asyncio.StreamReader, writer: asyncio.StreamWriter) -> None:
    try:
        while True:
...
async def start_image_server_loop(path: str = UNIX_CONNECTION_PATH) -> None:
    await asyncio.start_unix_server(_image_watcher_server_loop, path)
